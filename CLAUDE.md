...
| แก้ไข/เพิ่ม/ลบแถว | ✓ (เฉพาะแถวตัวเอง) | ✓ | **✕ — ดูอย่างเดียวทั้งหมด** |
...
- ปุ่มลบ (ฝั่งคลังสินค้า) **ต้องใส่รหัสคลังสินค้าทุกครั้ง ไม่ใช้ shortcut `unlocked` ของ session เหมือนปุ่มอนุมัติ** เพราะลบแล้วกู้คืนไม่ได้ · กดที่แถว `approved` แล้ว → alert ปฏิเสธทันที ไม่เปิด modal (ประวัติที่อนุมัติแล้วห้ามลบ) · `pwAction` state เป็นตัวบอก modal ว่าจะ approve หรือ delete — **ต้อง set ก่อน `setPwRowId` เสมอทั้ง 2 จุดเรียก** ไม่งั้นค่าค้างจากการกดครั้งก่อนจะทำผิด action
