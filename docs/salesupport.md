...
- **วัดจริงด้วย `App.css` ตัวจริง + ฟอนต์จริง (Chrome headless):** ตารางต้องการ **1,500px** (ลดจากเดิม **47%**)
  - ⚠️ ตัวเลข 1,500px นี้วัดตอนที่ `min` ของทุกคอลัมน์ยังไม่มีผล (กับดัก `!important` — ดูหัวข้อ "ความกว้างคอลัมน์") · หลังแก้ 2569-08-20 พื้นความกว้างตาม config คือ **~1,614px** ซึ่งยังพอดีจอ 1920

| จอ | ที่ว่าง | ผล |
|---|---|---|
| 1920 | 1,738px | ✅ พอดี เหลือ 238px |
| 1600 | 1,418px | ⚠️ ล้น 82px — ลากคอลัมน์ SKU (ค่าเริ่มต้น 320px) ให้แคบลงแล้วพอดี · ตอนนี้ลากคอลัมน์ไหนก็ได้ |
...
## ความกว้างคอลัมน์ — `min` ใน `MENUS` + ลากปรับได้ทุกคอลัมน์ (2569-08-20)

🚨 **บั๊กที่อยู่มานาน แก้แล้ว: `min` ใน `MENUS[].columns` ไม่เคยมีผลเลยสักคอลัมน์**

`App.css` เคยมี `.ss-table th { min-width: 60px !important }` — **กฎที่มี `!important` ในสไตล์ชีตชนะ inline style ธรรมดา** ค่าที่ JSX ส่งลงไปเป็น `style={{ minWidth: col.min }}` จึงถูกทิ้งทั้งตาราง ทุกคอลัมน์ยุบเหลือ 60px เท่ากันหมด (ตารางเป็น `table-layout: fixed` ที่เหลือเลยถูกหารเท่า ๆ กัน)

- **อาการที่เห็น:** เมนู Request Item ช่อง "ชื่อสินค้า" กับ "Generic Name" แคบเท่าคอลัมน์ `EXP` ข้อความยาวถูกบีบขึ้นบรรทัดใหม่ทีละไม่กี่ตัวอักษร
- **หลักฐานว่าเป็นแบบนี้จริง:** ทุกคอลัมน์ที่ "กว้างได้จริง" ต้องมีกฎ CSS ของตัวเองพร้อม `!important` เสมอ — `ss-col-status` 118px · `ss-col-availability` 132px · `ss-col-moq`/`ss-col-sku` 100px · `ss-col-sku_name` ผ่านตัวแปร `--sku-name-width` · `ss-delete-col` 54px · **ถ้า inline `min` ใช้ได้ กฎพวกนี้ไม่มีเหตุผลต้องมีอยู่**
- **แก้โดยถอด `!important` ออกจาก `.ss-table th` และ `.ss-table td`** แล้วย้ายตัวเลขจาก CSS ทั้งหมดกลับเข้า `min` ของ `MENUS` → **เหลือแหล่งความจริงเดียว** · กฎ CSS ต่อคอลัมน์ของ Request ถูกลบทิ้ง
- ⚠️ **ห้ามเขียนความกว้างคอลัมน์กลับลง CSS พร้อม `!important` อีก** — นอกจากจะทับ `MENUS` แล้ว ยัง**ทำให้ผู้ใช้ลากย่อคอลัมน์นั้นไม่ได้ด้วย** (ค่าที่ลากมาเป็น inline style เหมือนกัน)
- ⚠️ **`columnStyle()` ต้องใส่ `width` ไม่ใช่ `minWidth` อย่างเดียว** — `table-layout: fixed` คิดความกว้างจาก `width` ของแถวแรกเท่านั้น สเปคระบุว่า `min-width`/`max-width` ไม่มีผลกับ `table-cell` ในโหมดนี้ · ใส่แต่ `minWidth` แล้วทุกคอลัมน์จะยังหารเท่ากันเหมือนเดิม (กฎ CSS เดิมที่เคยใช้ได้ก็เขียน `width` คู่กับ `min-width` เสมอ)
- ⚠️ **`columnStyle()` ต้องใส่ทั้ง `th` และ `td`** — Order/BackOrder เป็น `table-layout: auto` ที่กางคอลัมน์ตามเนื้อหาของทุกเซลล์ ใส่แต่ `th` แล้วช่องข้อมูลยาว ๆ จะดันคอลัมน์กว้างกลับมาเอง · **`td` ของ status/availability/moq/sku เดิมไม่มีคลาส `ss-col-*` ด้วยซ้ำ** (เพิ่งเพิ่มให้ครบ) นี่คือเหตุผลที่ CSS เดิมเจาะจง `th` อย่างเดียวได้

**ผลข้างเคียงที่ตั้งใจ:** ตอนนี้ `min` มีผลกับ**ทุกเมนู** ไม่ใช่แค่ Request — Order/BackOrder เดิมพึ่ง auto layout ล้วน ๆ ตอนนี้มีพื้นความกว้างตาม config แล้ว (Order ~1,614px · BackOrder ~1,424px · ยังพอดีจอ 1920 ที่มีที่ ~1,738px)

**ลากปรับความกว้างได้ทุกคอลัมน์** — เดิมมีเฉพาะคอลัมน์ SKU ของ Order/BackOrder (ระบบ `--sku-name-width` + `skuNameWidth` state ซึ่ง**ถูกลบทิ้งแล้ว** เหลือระบบเดียว)

- state `colWidths: Record<string, number>` key = `` `${menuId}:${colKey}` `` → **ความกว้างแยกตามเมนู** เมนูเดียวกันคอลัมน์ชื่อซ้ำกันก็ไม่ชนกัน
- persist ลง `localStorage` key `ssColumnWidths` · `loadColumnWidths()` **กรองค่าที่ไม่ใช่ตัวเลข/นอกช่วง 60–700 ทิ้ง** — ความกว้างที่เป็น `NaN` ทำให้คอลัมน์หายทั้งคอลัมน์
- ⚠️ **ความกว้างเริ่มต้นตอนเริ่มลากอ่านจาก `th.getBoundingClientRect()` ไม่ใช่จาก state** — คอลัมน์ที่ยังไม่เคยถูกลากไม่มีค่าใน `colWidths` เลย ถ้า fallback เป็นค่าคงที่ คอลัมน์จะกระโดดทันทีที่แตะที่จับทั้งที่ยังไม่ได้ลากไปไหน
- แถบสีของที่จับโผล่เฉพาะตอน hover หัวคอลัมน์ — 20 คอลัมน์ที่มีขีดเทาค้างตลอดเวลาอ่านตารางยากขึ้นมาก
- **ไม่มีปุ่มรีเซ็ตความกว้าง** — เคยใส่ปุ่ม "↔ รีเซ็ตความกว้างคอลัมน์" ใน toolbar แล้วผู้ใช้ให้เอาออก (2569-08-20) อย่าใส่กลับ · ทางคืนค่าคือลากเอง (ต่ำสุด 60px คอลัมน์จึงไม่มีทางหายไปเลย) หรือล้าง `ssColumnWidths` ใน localStorage

**คอลัมน์ Request Item ที่ขยายตามคำสั่งผู้ใช้ 2569-08-20** (ดีไซน์ "Widen" แบบที่ 1 จาก `public/request-name-column-designs.html`): `product_name` 180→240 · `generic_name` 130→190 · `need_date` 130→150 · `note` 140→220 · `supplier` 140→220 · `customer_name` 120→170 — ตารางรวม ~2,574px ต้องเลื่อนแนวนอนทุกจอ (แลกกับข้อความครบ ผู้ใช้ลากย่อเองได้ถ้าไม่ชอบ)
...
- ช่อง "เบอร์โทรติดต่อ *" ในฟอร์ม ➕ Add BackOrder (ใต้ "ชื่อลูกค้า") — **บังคับกรอก** เหมือน `sale_bill_no`
  - ⚠️ **บังคับที่ฟอร์มเท่านั้น คอลัมน์ใน DB ยังเป็น nullable** — แถวที่สร้างก่อน migration `202608190003` ไม่มีเบอร์ ถ้าใส่ `not null` จะ migrate ไม่ผ่าน · ตารางจึงยังเจอ `—` ในบรรทัดล่างของ "ชื่อลูกค้า" ได้จากแถวเก่า
  - **ช่อง "ชื่อลูกค้า *" บังคับกรอกด้วย** (2569-08-20) — แถวค้างส่งต้องรู้ว่าติดต่อใครกลับได้ ทั้งคู่ nullable ใน DB ด้วยเหตุผลเดียวกัน
  - ⚠️ **ลำดับ `if` ใน `saveBackOrder` เรียงตามลำดับช่องในฟอร์ม** (sku → ค้างส่ง → ชื่อลูกค้า → เบอร์โทร → เลขที่บิล) ไม่ใช่ลำดับที่เขียนเพิ่มทีหลัง — ผู้ใช้จะได้เจอ error ของช่องบนก่อนช่องล่างเสมอ เพิ่มช่องบังคับใหม่ให้แทรกตามตำแหน่งจริงในฟอร์ม
