...
### Products — อัปโหลดอัตโนมัติทุกเช้า 08:30 (เพิ่ม 2569-09-04)

R05.106 ถูก export ใหม่ทุกเช้า → `upload-products.mjs` รันผ่าน Task Scheduler บนเครื่อง **Server (AninMainPC)** เวลา 08:30 · ปุ่มอัปโหลดหน้าเว็บยังอยู่เหมือนเดิมเป็นทางสำรอง
...
- flag: `--dry-run` (ตรวจอย่างเดียว) · `--force` (ข้าม guard วันที่ + ไฟล์หด)
