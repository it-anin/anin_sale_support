...
#### 🚨 RPC มีบรรทัดที่ repo นี้เพิ่มเข้าไป — re-run ไฟล์จาก repo บอทจะลบทิ้งเงียบ ๆ (2569-09-15)

`swap_products_from_import()` ถูกแก้ด้วยมือใน Supabase SQL Editor ให้เรียก `log_price_changes()` **1 บรรทัด ก่อน** `delete from products`:

```sql
begin
  perform public.log_price_changes();                -- ⬅️ บรรทัดที่ repo นี้เพิ่ม
  delete from public.products where id is not null;
  insert into public.products (...) select ... from public.products_import;
```

- **ทำไมต้องอยู่ก่อน delete** — หลัง delete แล้ว `products` ว่าง join ไม่เจออะไร → log ว่างเงียบ ๆ **ไม่มี error** เป็นวิธีพลาดที่เป็นไปได้มากที่สุด
- **ทำไมไม่ใช้ trigger** — `products` เขียนด้วย DELETE + INSERT ไม่ใช่ UPDATE → `AFTER UPDATE` ไม่มีทางยิงเลย · `AFTER INSERT ON products_import` ก็ไม่ได้เพราะบอทเขียนทีละ 500 แถว trigger จะเห็น staging ที่ยังโหลดไม่ครบ
- ⚠️ **`products-import-swap.sql` ใน [it-anin/botr05106](https://github.com/it-anin/botr05106) ไม่มีบรรทัดนี้** — รันไฟล์นั้นทับเมื่อไหร่ แจ้งเตือนราคาจะหยุดทำงานแบบไม่มีสัญญาณอะไรเลย (ไม่ error ไม่ log แค่ไม่มีแจ้งเตือนอีก) ต้องเพิ่มกลับทุกครั้ง
- ✅ รันใน transaction เดียวกับ swap → swap rollback (แบบ 2569-09-05) log ก็ rollback ด้วย ไม่มีแจ้งเตือนผีของรอบที่ไม่ได้ลงจริง
- นิยามของ `log_price_changes()` + ตาราง `price_change_log` / `price_change_seen` อยู่ที่ `price-change-setup.sql` (repo นี้)

#### คีย์เทียบราคาคือ `barcode` ไม่ใช่ `sku` หรือ `(sku, unit)` (วัดจริง 2569-09-15)

| คีย์ | ค่าไม่ซ้ำ | กลุ่มที่ซ้ำ | แถวในกลุ่มซ้ำ |
|---|---|---|---|
| `sku` | 7,959 | 2,485 | 5,390 |
| `(sku, unit)` | 10,830 | **34** | 68 |
| **`barcode`** | **10,864** | **0** | **0** |

`(sku,unit)` ซ้ำเพราะ SKU เดียวมี EAN 2 ตัวสำหรับแพ็กเดียวกัน (เช่น `100379|ขวด` → `8002660043375` / `8852796916032` ราคาเท่ากันทั้งคู่) — ใน 34 กลุ่มนั้นมี **2 กลุ่มที่ราคาต่างกัน** ซึ่งจะ diff กำกวมถ้าใช้ `(sku,unit)` · `barcode` ไม่ซ้ำเลยสักแถวและไม่มีแถวไหนว่าง จึงเป็นคีย์ธรรมชาติของแถว R05.106
...
- **ไม่มี FK ไป `products(sku)` โดยตั้งใจ:** (1) `products.sku` ไม่ unique (**7,959 unique จาก 10,864 แถว** — วัดใหม่ 2569-09-15 · `(sku, unit)` ก็ยังซ้ำ 34 กลุ่ม ดูหัวข้อคีย์เทียบราคาด้านบน) เป็น FK target ไม่ได้ (2) การอัปโหลด R05.106 ลบ `products` ทั้งตารางทุกครั้ง (ทั้งของเดิมที่หน้าเว็บ และ RPC swap ปัจจุบัน) → FK จะพังหรือ cascade ลบหมวดทิ้ง · INNER JOIN ใน view ซ่อน SKU กำพร้าให้อยู่แล้ว
